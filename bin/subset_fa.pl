...
my $usage = "\n$0\n" .
			"NO SPACES IN FASTA HEADERS PLEASE\n" .
			"\t-i <input gene list>\n" .
			"\t-f <fa file>\n" .
			"\t-o <output>\n" .
			...
			"\t--strip_before <keep everything after split character>\n" .
				"\t\t<dummy flag for viigf>\n" .
			"\t--strip_after <keep everything before the split character>\n" .
			"\t-s\n" .
				"\t\t<optional, specify comma separated list of headers instead of file>\n" .
...
while (my $line = <$fasta_fh>) {
	...
	if ($trans[1]) {
		...
		($line[0] = $line) =~ s/.*$strip_character//g if $strip_before;
		($line[0] = $line) =~ s/$strip_character.*//g if $strip_after;
		#remove carrot... I guess. thought this already happened.. oh whale
		($line[0] = $line[0]) =~ s/^>//g;
